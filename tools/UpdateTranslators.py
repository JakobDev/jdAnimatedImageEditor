...
def main() -> None:
    ...
    parse_translation_directory(root_dir / "jdAnimatedImageEditor" / "translations", "jdAnimatedImageEditor_", ".ts", translator_dict)
    parse_translation_directory(root_dir / "deploy" / "translations", "", ".po", translator_dict)

    # This Person contributed the Swedish translation before Weblate
    if "Åke Engelbrektson" not in translator_dict["sv"]:
        translator_dict["sv"].append("Åke Engelbrektson")
    ...
    if args.stdout:
        ...
        with open(root_dir / "jdAnimatedImageEditor" / "data" / "translators.json", "w", encoding="utf-8", newline="\n") as f:
